...
/obj/machinery/atmospherics/binary/circulator/on_update_icon()
	...
	if (last_pressure_delta > 0 && recent_moles_transferred > 0)
		if (temperature_overlay)
			overlays += image('icons/obj/machines/power/teg.dmi', temperature_overlay)
		if (last_pressure_delta > 5*ONE_ATMOSPHERE)
			overlays += image('icons/obj/machines/power/teg.dmi', "circ-run")
		else
			overlays += image('icons/obj/machines/power/teg.dmi', "circ-slow")
	else
		overlays += image('icons/obj/machines/power/teg.dmi', "circ-off")
